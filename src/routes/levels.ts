...
levelsRouter.post("/", authMiddleware, zValidator("json", levelCreateSchema), async (c) => {
  const body = c.req.valid("json");

  const rows = await db.insert(levels).values({
    index: body.index,
    title: body.title,
    text: body.text,
...
levelsRouter.put("/:uuid", authMiddleware, zValidator("param", uuidParamSchema), zValidator("json", levelUpdateSchema), async (c) => {
  const { uuid } = c.req.valid("param");
  const body = c.req.valid("json");
  ...
  const rows = await db.update(levels)
    .set({
      index: body.index ?? current.index,
      title: body.title ?? current.title,
      text: body.text ?? current.text,
      requires_subscription: body.requires_subscription ?? current.requires_subscription,
      updated_at: new Date(),
    })
...
levelsRouter.delete("/:uuid", authMiddleware, zValidator("param", uuidParamSchema), async (c) => {
  const { uuid } = c.req.valid("param");
